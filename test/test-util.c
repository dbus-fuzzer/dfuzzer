...
static void test_df_execute_external_command(void)
{
        for (guint8 i = 0; i < 2; i++) {
                ...
                g_assert_true(df_execute_external_command("true; echo hello world; true", show_output) == 0);
                g_assert_true(df_execute_external_command("true; echo hello world; false", show_output) > 0);
